...
bool oldState = HIGH;
int showType = 0;
  int pixelCount = 0; //strip.numPixels();
...
void loop() {
  ...
  extendTo(pixelCount, strip.Color(0, 127, 127), 20);
...
void extendTo(uint8_t max_pixels, uint32_t c, uint8_t wait) {
  for (int i=0; i < strip.numPixels(); i++) {
      if (i < max_pixels) {
        strip.setPixelColor(i, c);
        strip.show();
...
void theaterChase2(uint8_t max_pixels, uint32_t c, uint8_t wait, uint8_t cycles) {
